/**
 * accounting xml libro-diario — generate Libro Diario XML for SII
 */

import { Command } from "@effect/cli";
import { Effect, Option as O } from "effect";
import { generateLibroDiarioXml } from "@emisso/accounting";
import {
  OutputRenderer,
  CliError,
  resolveFormat,
...
export const xmlLibroDiarioCommand = Command.make(
  "libro-diario",
  options,
  ({ ledger, period, rut, razonSocial, output, format, json }) =>
    Effect.gen(function* () {
      ...
      const xml = yield* Effect.try({
        try: () => generateLibroDiarioXml(entries, {
          rut: company.rut,
          ...
          kind: "validation",
          message: "Failed to generate Libro Diario XML",
...
).pipe(Command.withDescription("Generate Libro Diario XML for SII"));
